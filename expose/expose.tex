...
%opening
\def\ThesisTitle{Implementation of an Enterprise Service Bus (ESB) with OpenShift und Camel}
\def\ThesisAuthor{Ing. Thomas Herzog B.Sc}
\def\ThesisLocation{Hagenberg}
\def\ThesisType{Master}
\def\CourseType{Master of Science}
\def\ThesisPubDate{\today} % Change here to the date you are going to print your thesis
\def\ThesisSupervisor{DI (FH) Peter Kulczycki}
\def\ThesisExternalSupervisor{Dr. Erhard Siegl}
\def\ThesisExternalCompany{Gepardec}
\def\ThesisSubject{}
...
\input{chapter/statement_of_the_problem}
\input{chapter/state_of_the_art}  
\input{chapter/deduction}  
\input{chapter/prototype}  
\input{chapter/empiric}    
\input{chapter/induction}  
